...
### 2026-04-28 — Mimari yeniden tasarım: Birleşik API Modeli + Asya domestic Gap Radar
**Karar zinciri (sırasıyla):**
1. TR pazaryerleri için Apify scraping kaldırıldı → resmi Satıcı API'leri (platform sahibi key + kayıtlı kullanıcı anonim havuzu) ile birleşik model
2. SerpAPI Trend Radar'dan kaldırıldı → birleşik API havuzu yeterli
3. Yeni fiyatlama matematiği yapıldı: 149/349/499 ₺ +KDV — Başlangıç ve Profesyonel %77-80 marj sağlam, **İşletme 499 ₺ "sınırsız" kotalarla matematiksel risk** (worst-case zarar)
4. Gap Radar resmi API ile çözülemedi (yurt dışı viral keşif resmi kanaldan elde edilemiyor)
5. **Kritik tespit (kullanıcı):** AliExpress batıya satış yapan ihracat platformu, ürün buraya gelene kadar zaten viralleşmiş → 6-12 ay geç sinyal. Doğru kaynak: **Asya domestic platformları** (Douyin / Xiaohongshu / Taobao / Coupang / Rakuten / Mercari JP) — 4-7 ay önde sinyal
6. **2x2 sınıflandırma matrisi (kullanıcı düzeltmesi):** "TR'de aranmıyor → ele" mantığı yanlış; aranmamak bilinmemek demek = keşif fırsatı. Eleme yalnız sol-alt karede (TR'de var + aranmıyor = ölü stok). Üç değerli kare: 🔥 SAFE GAP / ⚡ ORTA GAP / 💎 EARLY GAP
7. **Görsel politikası ayrıştırıldı (kullanıcı tespiti):** TR-içi ekranlardaki kısıt TR satıcılarını birbirinden korumaya yönelik; Gap Radar yabancı kaynaklarda bu mantık geçerli değil. **Bifurcated policy:** TR-içi (yalnız kendi API görseli, rakip A***L sansürlü) + Gap Radar (CDN proxy + attribution + 24 saat takedown ToS, görsel sunucuda host yok)

**Tamamlandı:**
- `docs/architecture.md` baştan yazıldı (12 bölüm):
  - §3 Birleşik API Modeli (TR pazaryerleri, anonim rıza, KVKK)
  - §4 Trend Radar Veri Akışı (TR-içi, SerpAPI/Apify yok, manuel kuyruk + e-posta akışı)
  - §5 Gap Radar Veri Akışı (Asya domestic, 5 adımlı pipeline, 2x2 matris, çeviri katmanı)
  - §6 Görsel Politikası ayrıştırılmış (6.1 TR-içi sıkı, 6.2 Gap Radar CDN proxy)
  - §7 Kaldırılan TR scraping özellikleri
  - §10 Maliyet özeti yeni fiyatlama ile: sıfır Gap Radar fazda $10-45/ay, Gap Radar aktif fazda $110-145/ay; break-even 15 İşletme abonesi
  - §12 Risk tablosu: Apify aktör çökmesi (yedekli chain), çeviri yanlışlığı, görsel takedown, "sınırsız" abuse uyarısı

**Karar verilmemiş (lansmandan önce yapılacaklar):**
- KVKK avukat incelemesi: anonim havuz rıza metni + Gap Radar CDN proxy hukuki tanımı (hostlama mı referans mı, eşdeğer içtihat)
- İşletme 499 ₺ planı için 3 seçenekten biri: (A) 799 ₺'ye çek, (B) "sınırsız"ı kaldır 300 yasal/200 tedarikçi cap, (C) 499 ₺ + aşım ücreti (1,5 ₺/ek kontrol)
- Apify hesabı + Asya kaynak aktör seçimi (her kaynak için 2 yedekli)
- SerpAPI hesabı (Developer $50)
- Pazarlama mesajı: "Asya domestic kaynak — 4-7 ay önde" pozisyonu (rakip TR araçları AliExpress kullanıyor, DropScout kaynaktan çekiyor)

**Kod tarafı dokunulmadı:** Bu oturum tamamen mimari planlama; mevcut backend kodu (Apify Gap Radar endpoint, SerpAPI Trends fetcher, image policy) lansman öncesi bu yeni mimariye göre revize edilecek.

**Sıradaki seçenekler:**
1. İşletme planı için A/B/C seçeneklerinden birini seç → pricing.html güncelle
2. KVKK avukat görüşmesi → rıza metni + ToS takedown maddesi yazma
3. Apify hesabı açıldığında Asya aktör ID seçimi + Secret Manager'a koy
4. Mevcut `gapRadar.ts` endpoint'ini Asya kaynak pipeline'ına revize et (5 adım: keşif → çeviri → arz → talep sınıflandırma → sunum)
5. Trend Radar'daki SerpAPI/Apify çağrılarını söndür (`lib/serpapi.ts`, `lib/marketplaceProducts.ts` Apify bağımlılıkları → API havuz fetcher'ı)

### 2026-04-28 (devam) — Trend Radar SerpAPI/Apify söndürüldü (Seçenek 5)
**Karar:** Mimari §4 doğrultusunda TR-içi Trend Radar artık scraping yerine resmi Satıcı API'leri + anonim kullanıcı havuzuna dayanacak. Havuz fetcher'ı henüz yazılmadı; bu oturumda eski scraping bağımlılıkları söndürüldü, endpoint "araştırma başlatıldı" yanıtına geçti.

**Tamamlandı:**
- `functions/src/lib/marketplaceProducts.ts` **silindi** — TR pazaryeri Apify scraper'ı (Trendyol/HB/N11/Amazon TR) tamamen kaldırıldı
- `functions/src/lib/trends.ts` **sadeleştirildi**:
  - `genPlaceholderSeries`, `summarize`, marketplace import/çağrıları kaldırıldı
  - `TrendSnapshot.source` artık tek değer: `'pool'` (eski `'serpapi' | 'placeholder'` yerine)
  - `fetchTrendForCategory(_)` artık `Promise<TrendSnapshot | null>` döner, body'si TODO ile `null` (anonim havuz hazır olunca burada platform sahibi API + opt-in havuz agregasyonu yapılacak)
  - `TRACKED_CATEGORIES`, `selectForTier`, `TrendDataPoint`, `TrendSnapshot` korundu (Gap Radar endpoint'i `TRACKED_CATEGORIES`'i hâlâ kullanıyor)
- `functions/src/lib/apify.ts` **TR pazaryeri secret'leri kaldırıldı**: `APIFY_TRENDYOL_ACTOR_ID`, `APIFY_HEPSIBURADA_ACTOR_ID`, `APIFY_N11_ACTOR_ID`, `APIFY_AMAZON_TR_ACTOR_ID` export'ları silindi. Kalanlar: `APIFY_TOKEN`, `APIFY_GAP_ACTOR_ID`, `APIFY_SUPPLIER_ACTOR_ID` (Gap Radar + Tedarikçi Bul için)
- `functions/src/schedulers/refreshTrends.ts` **silindi** — periyodik scrape cron'u. Yeni mimaride veri akışı kullanıcı tetiklemeli (manuel kuyruk + e-posta bildirimi); cron'a gerek yok
- `functions/src/schedulers/index.ts` — `refreshTrends` re-export'u çıkarıldı
- `functions/src/index.ts` — `scheduledRefreshTrends` re-export'u + `SERPAPI_KEY` + 4 TR actor secret bind'i `api` function'dan kaldırıldı; kalan secret'ler: `CLAUDE_API_KEY`, `ENCRYPTION_KEY`, `SENTRY_DSN`, `ALERT_WEBHOOK_URL`, `APIFY_TOKEN`, `APIFY_GAP_ACTOR_ID`, `APIFY_SUPPLIER_ACTOR_ID`
- `functions/src/endpoints/trends.ts` **yeniden yazıldı**:
  - Cache doc yoksa veya history boşsa `{ status: 'pending', message: 'Bu kategori için araştırma başlatıldı, tamamlandığında bilgilendirileceksiniz.', snapshot: null }` döner (eski 404 yerine)
  - Single-category response: `status: 'pending' | 'ready'` field'ı eklendi
  - All-categories response: tüm kategoriler boşsa top-level `status: 'pending'`, message dolu
  - `TRACKED_CATEGORIES` fallback'i artık eksik kategorileri tamamlar (tutarlı 15 kategori UI'ya gider)
- `lib/serpapi.ts` **dokunulmadı** — şu an hiçbir yerden import edilmiyor (Gap Radar revizyonu — Seçenek 4 — sırasında Asya kaynak TR talep sınıflandırma için bağlanacak); kullanılmayan dosya olarak repoda kalıyor
- Build: `cd functions && npm run build` ✓ (TS temiz); root `npm run build` ✓ (17 sayfa, trendRadar.js 22.90 kB ≈ değişmedi, 1.76s)

**Frontend dokunulmadı:**
- `trend-radar.html`'deki `shouldUseMock()` fallback zaten "items boş veya hiç products yok" → mock'a düşüyor; backend artık her zaman pending durumda olduğu için mock kalıcı olarak görünüyor (TEST VERİSİ rozeti). Yeni `status: 'pending'` field'ı görmezden geliniyor — kırılmıyor
- Manuel kuyruk e-posta akışı (mimari §4 adım 4-6) henüz UI'da yok; bir sonraki oturumda `trend-radar.html` "araştırma başlatıldı" banner'ı + `users/{uid}/trendQueue/{categoryId}` Firestore koleksiyonu eklenecek

**Deploy notları:**
- `firebase deploy --only functions` çalıştırıldığında Cloud Scheduler `firebase-schedule-scheduledRefreshTrends-europe-west1` job'u **otomatik silinir** (Functions discovery export edilmiyor)
- Secret Manager'da `SERPAPI_KEY`, `APIFY_TRENDYOL_ACTOR_ID`, `APIFY_HEPSIBURADA_ACTOR_ID`, `APIFY_N11_ACTOR_ID`, `APIFY_AMAZON_TR_ACTOR_ID` artık hiçbir Function'a bind değil — değer kalsa da kullanılmıyor; manuel temizlik isteğe bağlı (`firebase functions:secrets:destroy <NAME>`). SERPAPI_KEY Gap Radar revizyonunda yeniden bind edilebilir, silmemek mantıklı.

**Bilinçli olarak yapılmadı:**
- `lib/serpapi.ts` silinmedi — Gap Radar Asya pipeline'ında (Seçenek 4) TR talep sınıflandırma için kullanılacak
- Anonim havuz fetcher kodu yazılmadı — platform sahibi (DropScout TR) Trendyol/HB/Amazon TR/N11 hesap key'leri Secret Manager'a girilince + KVKK rıza akışı tasarlanınca yazılacak
- Manuel kuyruk + e-posta bildirimi (mimari §4 adım 4-6) — backend cron + email provider seçimi gerek; ayrı oturum
- `cache/trends/items` Firestore koleksiyonundaki eski snapshot/history doc'ları temizlenmedi — yeni `source: 'pool'` field'ı eski `'serpapi'`/`'placeholder'` doc'larıyla type-uyumsuz, ama endpoint snapshot'ı doğrudan dönüyor (frontend tolere ediyor); ileride manuel bir migration/temizlik yapılabilir

**Sıradaki seçenekler:**
1. Mimari §4 manuel kuyruk + e-posta akışı — `users/{uid}/trendQueue/{categoryId}` koleksiyonu, "araştırma başladı"/"tamamlandı" e-posta tetikleyicisi (Postmark/SendGrid + Functions trigger)
2. Anonim havuz fetcher iskeleti — `users/{platform-owner-uid}/platforms/{trendyol}` snapshot'ından başlangıç verisi (rıza metni gelene kadar diğer kullanıcılar dahil edilmez)
3. Seçenek 4 — `gapRadar.ts` Asya pipeline revizyonu (Apify hesabı + actor ID gerekiyor)
4. Seçenek 1 — İşletme planı A/B/C kararı + pricing.html güncellemesi
5. Onceki listede kalan: KVKK avukat (Seçenek 2), Apify hesabı (Seçenek 3)

**Deploy:** `firebase deploy --only functions,hosting` (refreshTrends cron otomatik silinir; frontend değişmedi ama hosting'i de yenileyince Vite çıktısı senkron kalır)

### 2026-04-29 — Gap Radar Asya domestic pipeline revizyonu (Seçenek 3)
**Karar:** Mimari §5 doğrultusunda Gap Radar yurt dışı viral kaynaklara (Asya domestic) bağlandı. Eski TR pazaryeri (Trendyol/HB/N11) Apify scraper kaldırıldı. Apify + SerpAPI ödemeleri yapılmadığı için sistem **placeholder-first** mantığıyla kuruldu: secret yoksa deterministic test verisi, hesap aktif olunca tek deploy ile gerçek veriye geçiyor.

**Tamamlandı (backend):**
- `functions/src/lib/apify.ts` — 6 Asya kaynak secret tanımı (`APIFY_DOUYIN_ACTOR_ID`, `APIFY_XIAOHONGSHU_ACTOR_ID`, `APIFY_TAOBAO_ACTOR_ID`, `APIFY_COUPANG_ACTOR_ID`, `APIFY_RAKUTEN_ACTOR_ID`, `APIFY_MERCARI_JP_ACTOR_ID`) + `AsianSourceId` tipi + `getAsianActorId(source)` accessor (env yoksa null → placeholder fallback)
- `functions/src/lib/asianSources.ts` (yeni) — 6 kaynağın adapter'ları:
  - `ASIAN_SOURCES` registry (id/name/country/lang/blurb)
  - `RawCandidate` tipi: sourceId, sourceUrl, externalId, title (orijinal), titleTr (önceden çevrilmişse), sourceLang, image (CDN URL), creator, metric{posts,likes,sales,rank,rating}, isPlaceholder
  - `discoverAsianCandidates(categoryId, query)` → 6 kaynak `Promise.allSettled` ile paralel; biri çökerse diğerleri akar
  - Her kaynak için `fetchOneSource`: token + actorId varsa `runActorSync`, yoksa `makePlaceholder`
  - `PLACEHOLDER_PRODUCTS` — 15 kategori × 8 ürün TR adı havuzu (Asya'ya yakışan tipte: kablosuz dudak nemlendirici, LED yüz maskesi, Pop-it klavye, konjac eriştesi vs.)
  - `ORIGINAL_HINT` — zh/ja/ko için "yapay ama dil rozetini doğru gösteren" başlık üreticisi (便携 / 新作 / 인기 prefix'leri)
  - `normalizeApifyItem` — gerçek aktör çıktısı için esnek alan adı eşleme (id/url/title/image/posts/likes/sales/rank/rating tahminleri); aktör seçilince iterasyon gerekebilir
- `functions/src/lib/claude.ts` — yeni `translateProductName(input)` helper'ı: Claude Haiku 4.5 + `provide_translation` tool-forced JSON, sistem promptu prompt-cache'li, `AsianLang` ('zh'|'ja'|'ko') desteği, ~100 token/ürün
- `functions/src/lib/gapPipeline.ts` (yeni) — 5 adımlı orchestrator:
  - **Adım 1 KEŞIF**: `discoverAsianCandidates` (6 kaynak paralel)
  - **Adım 2 ÇEVİRİ**: `translateBatch` — placeholder zaten çevirili (skip), gerçek aktörden gelen orijinal başlıklar Claude'a; bütçe `TRANSLATE_BUDGET_PER_CATEGORY = 25` (~2.5K token/kategori, $0.001 mertebesi); per-item 8s timeout; hata durumunda orijinal başlık korunur
  - **Adım 3 TR ARZ**: `checkTrSupply` — anonim havuz boş, şu an `null` döner (TODO: havuz aktif olunca platform sahibi key'i ile keyword search)
  - **Adım 4 TR TALEP**: `fetchTrDemand(query)` — `hasSerpApiKey()` varsa `fetchGoogleTrendSeries` (son 30g + son 7g eğilim); yoksa deterministic seed-based skor (30-90 aralık) `direction: 'up'|'flat'|'down'`
  - **Adım 5 SINIFLAMA**: 2x2 matris — `supplyCount > 0 + trendScore >= 40` → orta; `> 0 + < 40` → eleme; `null/0 + >= 40` → safe; `null/0 + < 40` → early. Havuz boşken `supplyCount = null`, dolayısıyla yalnız safe + early karelerine düşer
  - `calcOpportunityScore` — sınıf bazlı baz (safe 78 / early 68 / orta 55 / eleme 25) + sosyal/marketplace sinyal × 0.25 + (trend - 50) × 0.3
  - `mode` raporlama: `'live' | 'placeholder' | 'mixed'` (placeholderRatio'ya göre)
- `functions/src/endpoints/gapRadar.ts` — baştan yazıldı:
  - Eski şema (`GapRadarItem[]` + opportunityScore/keywords/sampleUrls) tamamen kaldırıldı
  - Yeni response: `{ snapshot: { groups: { safe[], orta[], early[], eleme[] }, meta, fetchedAt, categoryId, categoryName }, cached }`
  - `cache/gapRadar/items/{categoryId}` 7g TTL (mevcut), eski `1g TTL` artırıldı (Asya pipeline daha pahalı)
  - Pipeline hatası → 502 `pipeline_failed` (önce sessizce placeholder'a düşürmüyor — şeffaf hata)
- `functions/src/index.ts` — 6 yeni Asya secret + `SERPAPI_KEY` `api` function'a bind edildi

**Tamamlandı (frontend `gap-radar.html`):**
- Sayfa baştan yazıldı (~870 satır). CSS pattern korundu (sidebar/topbar/theme), ana içerik + script tamamen yeni
- **KPI strip 4 yeni kart**: 🔥 SAFE GAP / ⚡ ORTA GAP / 💎 EARLY GAP / Toplam Aday (+ kaynak ve aday sayısı sub-text)
- **Hero kartı yeniden yazıldı** — "2x2 sınıflandırma matrisi" başlığı, kaynak listesi, 3 mini özet (safe/orta/early count); signal-pill'lar artık `Mod / Tazelik / Talep kaynağı`
- **Mode banner** — sayfa üstünde renkli bant: 🟡 TEST MODU (placeholder), 🟦 KARMA MOD (mixed), ✅ CANLI VERİ (live). Kullanıcı veri kaynağını anında görür
- **Tab strip** — 3 sekme `🔥 SAFE / ⚡ ORTA / 💎 EARLY` + tab-count badge'leri (her sekmede kaç ürün var); aktif sekme mavi-mor gradient. Eleme sekmesi UI'da yok (dahili)
- **Filter bar** — kategori dropdown + **kaynak dropdown** (`all` + 6 Asya kaynak) + min skor filter
- **Ürün kartı baştan**:
  - Sol: 88×88 thumb (item.image varsa CDN'den `<img loading="lazy">`, yoksa kategori emoji); sol-altta `🇨🇳/🇰🇷/🇯🇵` bayrak chip
  - Orta: titleTr (Türkçe başlık, büyük) + orijinal başlık (gri italic, "orijinal:" prefix), descriptionTr açıklama, kaynak attribution chip (mor), kategori chip, TR Talep chip (skor + ↑/↓/→ veya "tahmini")
  - Sağ: score-ring (sınıfa göre yeşil/sarı/mor) + cls-chip (🔥 SAFE / ⚡ ORTA / 💎 EARLY)
  - Metrik grid 4 kutu — sosyal (posts/likes) vs marketplace (sales/rank) otomatik switch + score + rating
  - Footer: attribution metni `Görsel: Douyin · @creator · tarih · kaynağa git ↗` + Takibe al/AI fırsat butonları
  - Placeholder ürünler için `TEST VERİSİ` rozeti
- **AI Brief paneli** — mevcut analyzeProduct entegrasyonu korundu; metrik chip'leri (TR Trend / Sınıflandırma / Sosyal sinyal / Kaynak) yeni şemaya göre güncel
- **Görsel politikası §6.2 uygulanmış**: image src direkt platform CDN'inden, attribution + kaynağa git linki her kartta, takedown ToS kapsamında

**Build:** `cd functions && npm run build` ✓ (TS temiz); root `npm run build` ✓ (17 sayfa, gap-radar.html 33.33 kB, gapRadar.js 17.59 kB; 1.75s)

**Şu anki test akışı (placeholder modu):**
1. Login → Business plan kullanıcısı (gateFeature business gate)
2. `gap-radar.html` aç → "🟡 TEST MODU" bandı görünür, KPI'lar dolar (15 kategori × 6 kaynak × 3 placeholder = 270 aday)
3. Kategori değiştir → ~1-2s pipeline (placeholder hızlı), 3 sekme dolar (genelde safe + early; orta/eleme boş çünkü TR ARZ null)
4. Sekme değiştir, kaynak filtresi (Douyin/Coupang vs.) → liste anında filtrelenir
5. Ürün kartında "AI fırsat analizi" → analyzeProduct endpoint (Claude Haiku) → sağ panele içgörü gelir
6. Aynı kategoriye geri dönünce 7g cache hit (`cached: true`)

**Apify/SerpAPI ödemesi yapıldığında geçiş prosedürü (otomatik):**
1. Apify hesabı aç + 6 Asya aktör seç (her platform için 1 aktör; yedekli için ileride 2 yapılabilir):
   - Douyin scraper, Xiaohongshu scraper, Taobao Best, Coupang Best, Rakuten Ranking, Mercari hot
2. `firebase functions:secrets:set APIFY_DOUYIN_ACTOR_ID` … (her aktör için tekrar; tipik format `username/actor-name`)
3. SerpAPI Developer ($50/ay) hesap aç → `firebase functions:secrets:set SERPAPI_KEY`
4. `firebase deploy --only functions:api` → secret bind hazır
5. Cache invalidation: `cache/gapRadar/items` koleksiyonunu Firestore Console'dan boşalt (veya 7g bekle); sonraki istek pipeline'ı taze çalıştırır
6. Mode banner otomatik 🟡 → ✅ döner; aktör çıktısının alan adları beklenenden farklıysa `lib/asianSources.ts → normalizeApifyItem` ince ayarı gerekebilir (tek dosya, ~50 satır)

**Tasarım kararları:**
- **Placeholder = pipeline ile aynı yol**: backend mode'u algılayıp aynı response şemasını üretiyor → frontend kodda hiçbir if/else gerekmiyor; mode banner sadece bilgilendirme
- **Tek kategori sorgusu = tek SerpAPI çağrısı**: TR talep kategori bazında çekiliyor (her ürün için ayrı değil) → maliyet 1/12 oranında düşük; hassasiyet kaybı kabul edilir (kategori-içi ürünler için tek talep skoru ortak)
- **TR ARZ null = sadece SAFE+EARLY**: havuz boşken doğru sınıflandırma "TR'de var mı bilmiyoruz" demektir; varsayım "TR'de yok" yapıldı (mantık: Asya viral ürünleri çoğunlukla TR'de gerçekten yok); havuz aktif olunca otomatik düzeltilir, ORTA + ELEME kareleri dolar
- **Çeviri bütçesi 25 ürün/kategori**: gerçek aktör çıktısında ~18 ürün geliyor varsayımıyla (6 kaynak × 3 sample); placeholder'da bütçe tüketilmiyor
- **Görsel CDN proxy YOK**: image attribute direkt platform URL'i; mimari §6.2 "DropScout sunucusunda hostlanmaz" politikası birebir uygulandı
- **Eski `APIFY_GAP_ACTOR_ID` secret korundu**: legacy `requireGapActorId` accessor'ı hâlâ var ama hiçbir yerden çağrılmıyor; ileride manuel temizlik

**Bilinçli olarak yapılmadı:**
- Aktör başına 2 yedekli sistem (mimari §5.1) — ilk MVP'de 1 aktör/platform; yedek ileride `APIFY_DOUYIN_ACTOR_ID_FALLBACK` eklenip `getAsianActorId` chain'lenir
- Asya kaynakları için kategori → query mapping (zh/ja/ko'da TR kategori adı çalışmaz) — şimdilik `category.query` (TR kelime) gönderiliyor; gerçek aktörler kategori ID veya çoklu dil destekleyebilir
- Cache aktör çıktılarını ayrı katmanlara bölme (ham aktör cache → çeviri cache → pipeline cache) — şu an pipeline çıktısı tek doc; çeviri tek seferlik olduğu için maliyet zaten düşük
- Trend Radar'a "Gap Radar'dan keşfet" linki — ileride Trend Radar kategori kartına Asya viral cross-link eklenebilir
- KVKK rıza metni / ToS takedown maddesi — frontend'de görsel attribution var ama yasal metin (mimari §6.2) henüz pricing/terms sayfasına eklenmedi; lansmandan önce avukat incelemesi gerekiyor

**Sıradaki seçenekler:**
1. Apify hesabı + 6 Asya aktör seçimi → secret rotate (kullanıcı hazır olduğunda)
2. SerpAPI Developer hesabı ($50/ay) → SERPAPI_KEY rotate (TR talep canlıya geçer)
3. Mimari §4 manuel kuyruk + e-posta akışı (Trend Radar için) — Postmark/SendGrid + Functions trigger
4. İşletme planı A/B/C kararı + pricing.html güncellemesi
5. Anonim havuz fetcher iskeleti (TR ARZ kontrolü için) — KVKK rıza akışı + platform sahibi key
6. KVKK avukat görüşmesi — anonim havuz + Gap Radar görsel politikası rıza/ToS metinleri

**Deploy:** `firebase deploy --only functions:api,hosting`
- 6 yeni Asya secret + SERPAPI_KEY secret bind'leri otomatik attach edilir (boş secret'ler problem değil — kod fallback'lerle çalışır)
- Cache temizlemek istenirse: Firestore Console → `cache/gapRadar/items` → manuel sil (ya da 7g bekle, TTL policy zaten aktif)
