...
// Gelistirme ortaminda emulator kullan
// Vite dev server (5173) veya Firebase Hosting emulator (5000) uzerinden geldiginde aktif olur
if (location.hostname === 'localhost' || location.hostname === '127.0.0.1') {
  // Hot-reload'da ikinci kez baglanmayi engelle
  if (!globalThis.__dropscoutEmulatorsConnected) {
    connectAuthEmulator(auth, 'http://127.0.0.1:9099', { disableWarnings: true });
    connectFirestoreEmulator(db, '127.0.0.1', 8080);
    globalThis.__dropscoutEmulatorsConnected = true;
    console.info('[DropScout] Firebase emulators connected (auth:9099, firestore:8080)');
  }
}
